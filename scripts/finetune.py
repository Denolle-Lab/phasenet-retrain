#!/usr/bin/env python3
"""
finetune.py  —  GPU-saturating fine-tune of jma_wc PhaseNet.

Optimisations
-------------
  * All waveforms pre-loaded into RAM → zero HDF5 I/O during training
  * AMP (fp16 forward / backward)     → ~2× GPU throughput
  * torch.compile()                   → kernel fusion, ~20-30% extra speedup
  * pin_memory + non_blocking         → async CPU→GPU transfers
  * Batch size 1024                   → fills RTX 3090 VRAM
...
def save_checkpoint(model, optimiser, scaler, epoch, val_loss, path: Path):
    ...
    # Save the underlying (non-compiled) module so checkpoints are portable
...
def run_epoch(model, loader, device, optimiser=None, scaler=None, grad_clip=1.0):
    ...
    optimiser != None  →  training mode (uses AMP scaler).
    """
...
def _plot_progress(metrics_csv: str, log_cfg: dict):
    ...
    out_dir = (
        ...
        / log_cfg.get("run_name", "finetune_jma_wc_noise")
    )
...
def train(config: dict, resume_path=None, init_from=None):
    ...
    device = torch.device(
        "cuda"
        if torch.cuda.is_available() and hw_cfg.get("accelerator", "auto") != "cpu"
        else "cpu"
    )
    ...
    ckpt_dir    = Path(log_cfg.get("checkpoint_dir", "checkpoints/finetune_jma_wc"))
    best_path   = ckpt_dir / "best.pt"
    last_path   = ckpt_dir / "last.pt"
    metrics_csv = log_cfg.get("metrics_csv", "results/finetune_jma_wc_metrics.csv")
    ...
    if resume_path:
        ...
        start_epoch += 1
        # Override LR — loaded optimizer state has old LR; use config value
...
def main(args):
    ...
    out_dir = (
        Path(log_cfg.get("save_dir", "results"))
        / log_cfg.get("run_name", "finetune_jma_wc")
    )
