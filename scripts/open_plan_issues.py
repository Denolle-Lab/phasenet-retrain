...
The training plan (docs/2026-09-07_training_plan.md v2) as GitHub issues,
grouped by milestone, with dependencies and acceptance criteria. One source:
this file renders docs/2026-09-09_issue_plan.md and, with --create, opens the
milestones, labels and issues on Denolle-Lab/phasenet-retrain through `gh`.
Nothing is created without --create.

    python scripts/open_plan_issues.py            # render the markdown only
    python scripts/open_plan_issues.py --create   # also open them on GitHub
"""
...
REPO = "Denolle-Lab/phasenet-retrain"
BRANCH = "audit/2026-09-07-generalization"
REPO_ROOT = Path(__file__).resolve().parent.parent
DOC = REPO_ROOT / "docs" / "2026-09-09_issue_plan.md"

MILESTONES = [
    ("Phase 0: evaluation before training", "Weeks 1-3. Task 1 on the server, baselines on the held-out set, the event-level scorer, thresholds by region."),
    ("Phase 1: signal and noise corpora", "Weeks 4-11. Bulletin-labelled corpus at the scale that made jma_wc good, the eleven-flavour noise corpus, augmentation, frozen suites."),
    ("Phase 2: training experiments", "Weeks 12-22. The data-scaling curve first, then init/anchor, noise ablation, width and context. PhaseNet only."),
    ("Phase 3: distant P and deployment", "Weeks 22-26. The 20 Hz distant-P model, thresholds by weight and region, the standing acceptance test."),
]
LABELS = [("plan", "0e8a16", "From the 2026-09 training plan"), ("server", "5319e7", "Needs the lab server (SeisBench cache, checkpoints, GPU)"),
          ("needs-access", "d93f0b", "Blocked on an account, an email or a data request"), ("cross-repo", "fbca04", "Touches QuakeScope as well")]

# (milestone index, title, labels, body)
ISSUES = [
    (0, "[Phase 0] Run the held-out sequence exclusion on the server and commit the list", ["plan", "server"], """
...
**Steps** (on the server, branch `{branch}`):
```bash
...
"""),
    (0, "[Phase 0] Server-side hypothesis tables: S fraction by distance (H2) and the full noise-pool ordering (H00)", ["plan", "server"], """
...
"""),
    (0, "[Phase 0] Rebuild the held-out test set on the server and score the existing weights at matched budget", ["plan", "server"], """
...
"""),
    (0, "[Phase 0] Event-level scorer: PyOcto association against the reference catalogues", ["plan"], """
...
"""),
    (0, "[Phase 0] Obtain the restricted picks and waveforms", ["plan", "needs-access"], """
...
"""),
    (0, "[Phase 0] Thresholds per weight and per region from quiet-day false-pick rates", ["plan", "cross-repo"], """
...
"""),
    (1, "[Phase 1] Operator bulletin census: how many manual picks each FDSN event service can give", ["plan"], """
...
**Depends on.** Nothing.
"""),
    (1, "[Phase 1] Bulletin-labelled corpus builder: 60 s windows with every arrival labelled", ["plan", "server"], """
...
"""),
    (1, "[Phase 1] Label quality: confident learning plus physical checks per operator", ["plan", "server"], """
...
"""),
    (1, "[Phase 1] Noise corpus: eleven flavours harvested by catalogue exclusion, held out by station", ["plan", "server"], """
...
"""),
    (1, "[Phase 1] Augmentation module: class-balanced real noise, event superposition, artefacts, resampling, random position", ["plan"], """
...
"""),
    (1, "[Phase 1] Freeze the development and acceptance suites and enforce the never-read rule", ["plan"], """
...
**Depends on.** Nothing.
"""),
    (2, "[Phase 2] E1: the data-scaling curve, before any recipe work", ["plan", "server"], """
...
"""),
    (2, "[Phase 2] E2: initialisation and anchor at the size E1 chose", ["plan", "server"], """
...
"""),
    (2, "[Phase 2] E3: noise ablation", ["plan", "server"], """
...
"""),
    (2, "[Phase 2] E4: PhaseNet width and input length", ["plan", "server"], """
...
"""),
    (2, "[Phase 2] The acceptance run and the QuakeScope notebooks", ["plan", "server", "cross-repo"], """
...
"""),
    (3, "[Phase 3] Distant-P model at 20 Hz on 120 s windows for offshore events and sparse regions", ["plan", "server"], """
...
def render() -> str:
    out = [f"# Training plan as GitHub issues\n", f"*2026-09-09. Rendered by `scripts/open_plan_issues.py`; `--create` opens them on `{REPO}`. Branch `{BRANCH}`.*\n"]
    for mi, (title, desc) in enumerate(MILESTONES):
        out.append(f"\n## {title}\n\n*{desc}*\n")
        for n, (m, t, labels, body) in enumerate(ISSUES, 1):
            if m == mi:
                out.append(f"\n### {t}\n\n`{'`, `'.join(labels)}`\n" + body.format(branch=BRANCH).rstrip() + "\n")
    return "\n".join(out)
...
def gh(*args, check=True):
    ...
    if check and r.returncode:
        print(r.stderr.strip()); sys.exit(r.returncode)
    return r.stdout.strip()


def create():
    existing = {l["name"] for l in json.loads(gh("label", "list", "-R", REPO, "--json", "name", "--limit", "100"))}
    for name, color, desc in LABELS:
        if name not in existing:
            gh("label", "create", name, "-R", REPO, "--color", color, "--description", desc); print("label", name)
    ms = {m["title"]: m["number"] for m in json.loads(gh("api", f"repos/{REPO}/milestones?state=all"))}
    for title, desc in MILESTONES:
        if title not in ms:
            m = json.loads(gh("api", f"repos/{REPO}/milestones", "-f", f"title={title}", "-f", f"description={desc}"))
            ms[title] = m["number"]; print("milestone", title)
    open_titles = {i["title"] for i in json.loads(gh("issue", "list", "-R", REPO, "--state", "all", "--limit", "200", "--json", "title"))}
    for m, t, labels, body in ISSUES:
        if t in open_titles:
            print("exists ", t); continue
        url = gh("issue", "create", "-R", REPO, "--title", t, "--body", body.format(branch=BRANCH).strip(),
                 "--milestone", MILESTONES[m][0], *sum((["--label", l] for l in labels), []))
        print("created", url)


def main():
    ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--create", action="store_true", help="Open milestones, labels and issues on GitHub")
    a = ap.parse_args()
    DOC.write_text(render()); print(f"wrote {DOC} ({len(ISSUES)} issues, {len(MILESTONES)} milestones)")
    if a.create:
        create()
